#!/usr/bin/env python3
"""
Daglig podkast-klipp generator og e-post sender
...
Her er dagens podkast-klipp klar for sosiale medier:
...
"""
    ...
    for podcast_key, info in clips_info.items():
        ...
        for i, clip in enumerate(info['clips'], 1):
            body += f"  {i}. {clip['type']} ({clip['start']}s - {clip['end']}s)\n"
    
    body += f"""

Klippene er i videoformat (1080x1920) klare for Instagram/TikTok.
    ...
    for podcast_key, info in clips_info.items():
        for clip in info['clips']:
            ...
            if os.path.exists(video_path):
                ...
                filename = f"{podcast_key}_{clip['type']}.mp4"
...
def process_podcast(podcast_key, processed):
...
def main():
    parser = argparse.ArgumentParser(description='Daglig podkast-klipp generator')
