...
def fetch_nielsen_data_with_retry():
    """Hent data fra Nielsen API med retry-logikk"""
    attempt = 1
    last_error = None
    
    while attempt <= MAX_RETRIES:
        try:
            print(f"🌐 Attempt {attempt}/{MAX_RETRIES}: Henter data fra Nielsen API...")
...
def main():
    print("📻 NIELSEN RADIO DATA - EKTE DATA FRA API (med retry)")
    print("=" * 60)
    print()
    
    # Hent data med retry
    data = fetch_nielsen_data_with_retry()
